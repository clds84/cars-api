// seed.js is going to be a script that we can run from the terminal, to create a bunch of cars at once. 

// we'll need to be careful with our seed here, and when we run it, because it will remove all the cars first, then add the new ones. 
...
mongoose.connect(db, {
    ...
    .then(() => {
        // then we remove all the cars except the ones that have an owner
        Car.deleteMany({ owner: null })
            .then(deletedCars => {
                console.log('deleted cars', deletedCars)
                // then we create using the startCars array
